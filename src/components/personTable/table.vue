  ...
  <div>
    <table cellspacing="0">
      ...
      <tr class="content" v-for="(item,index) in tableData" :key="item.id">
        ...
        <td @click="enterCom(item.school_id)">
          <a
            href="javascript:;"
            v-bind:class="[curSchoolId == item.school_id ? 'activeClass' : '']"
          >{{curSchoolId == item.school_id ?'当前企业':'进入企业'}}</a>
        </td>
      </tr>
    </table>
          ...
          .then(res => {
            ...
            this.$api.toLogin
              ...
              .then(res => {
                ...
                this.$store
                  .dispatch("getRole", res.data.data.user_role)
                  .then(res => {
                    if (_res.data.data.user_role == "admin") {
                      this.$router.push({
                        path: "companyManage",
                        query: { changeSchool: "1" }
                      });
                    } else {
                      this.$router.push({
                        path: "studyCenter",
